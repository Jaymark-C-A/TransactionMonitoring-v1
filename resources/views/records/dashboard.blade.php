    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (filteredData.length > 0) {
                            ...
                            serviceStartTime = null; // Reset the service start time
                        }
                    },
                    error: function(xhr, status, error) {
            ...
            function updateServiceTime() {
                if (serviceStartTime) {
                    var now = new Date();
                    var elapsedTime = Math.floor((now - serviceStartTime) / 1000); // Elapsed time in seconds
            ...
            $('.current-ticket .actions button.next').click(function() {
                $.ajax({
                    ...
                    success: function(response) {
                        fetchData();
                        serviceStartTime = new Date(); // Reset service start time for the next ticket
                        $.ajax({
            ...
            // Update the service time display every second
